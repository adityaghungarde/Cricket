...
int main(){
    ...
    float Avg[n];
void Accept(struct Player_Info p[10],int n);
void Display(struct Player_Info p[10],int n);
void Country_Category(struct Player_Info p[10] , int n , char Country[20],int category);
...
void Accept(struct Player_Info p[5], int n){
    for(int i=0; i<n ; i++){
        printf("Input details of Player %d",i+1);
        printf("\nEnter Player Name : ");
        scanf("%s",p[i].Name);
